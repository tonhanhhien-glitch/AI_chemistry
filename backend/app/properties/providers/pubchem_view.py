...
class PubChemViewPropertyProvider:
    ...
    def _normalize(self, payload: dict[str, Any], query: PropertyQuery) -> list[NormalizedProperty]:
        ...
        for heading, (key, category, label_vi, label_en) in _HEADINGS.items():
            ...
            if section is None:
                continue
            text, reference = _section_text(section)
            if not text:
                continue
            if key in BULK_PHASE_PROPERTY_KEYS and query.is_ion and not _describes_exact_entity(text, query):
            ...
            properties.append(NormalizedProperty(
                ...
                value=text, evidence_type=PropertyEvidenceType.EXPERIMENTAL,
                source_name="PubChem", source_reference=reference, source_url=url,

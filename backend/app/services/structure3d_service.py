...
def _from_block(record: dict[str, Any], data: str, *, source: StructureSource, source_label: str, data_format: str, pubchem_cid: int | None = None) -> Structure3D | None:
...
def _idealized(record: dict[str, Any]) -> Structure3D:
    ...
    label = "Idealized VSEPR model"
    return Structure3D(
        atoms=atoms, bonds=bonds, source=StructureSource.IDEALIZED_VSEPR,
        source_label=label, is_illustrative=True, is_computed=False,
        is_experimental=False, central_atom_id="a0",
        angle_annotations=_angle_annotations(atoms, bonds, "a0", "ideal_vsepr", label),
...
def resolve_structure3d(record: dict[str, Any]) -> Structure3DResult:
    ...
    if cid:
        ...
        if pubchem.data:
            structure = _from_block(record, pubchem.data, source=StructureSource.PUBCHEM_3D, source_label="PubChem 3D conformer", data_format="sdf", pubchem_cid=int(cid))
    ...
    if rdkit.structure:
        structure = _from_block(record, rdkit.structure.molblock, source=StructureSource.RDKIT_ETKDG, source_label=f"RDKit-generated conformer ({rdkit.structure.force_field})", data_format="molblock")
        if structure is not None:
            return Structure3DResult(structure, tuple(statuses))
    return Structure3DResult(_idealized(record), tuple(statuses))

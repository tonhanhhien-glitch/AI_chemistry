...
export default function VSEPRCard({ result }: { result: VseprResult }) {
  ...
  return <div className="vsepr-card"><div className="notation-row"><AXENotationBadge notation={result.ax_en} /><p>{t("vsepr.notationHint")}</p></div><ElectronDomainTable result={result} /><div className="geometry-grid"><GeometryCard title={t("vsepr.electronGeometry")} geometry={result.electron_geometry} geometryVi={geometryLabel(t, result.electron_geometry)} /><GeometryCard title={t("vsepr.molecularGeometry")} geometry={result.molecular_geometry} geometryVi={geometryLabel(t, result.molecular_geometry)} angle={result.ideal_angle} /></div>{distortionNote && <p className="callout">{distortionNote}</p>}<p>{teachingNote}</p>{result.pedagogical_hybridization && <p className="muted">{t("vsepr.pedagogicalLabel")} <strong>{result.pedagogical_hybridization}</strong>. {hybridizationWarning}</p>}</div>;
}

...
describe("AnalysisPage", () => {
  ...
  it("renders the full offline analysis and WebGL fallback", async () => { vi.mocked(analyzeMolecule).mockResolvedValue(waterAnalysis); renderPage(); await userEvent.type(screen.getByLabelText("Công thức hoặc tên chất"), "H2O"); await userEvent.click(screen.getByRole("button", { name: "Tìm" })); expect(await screen.findByRole("heading", { name: "Cấu trúc Lewis", level: 2 })).toBeInTheDocument(); expect(screen.getAllByText((_, element) => element?.textContent === "AX2E2").length).toBeGreaterThan(0); /* AX2E2's "2"s render as subscript elements, splitting the text across nodes */ expect(screen.getByText(/Trình duyệt không thể khởi tạo WebGL/)).toBeInTheDocument(); await userEvent.click(screen.getByRole("button", { name: /Giải thích sư phạm/ })); /* The explanation panel renders the deterministic sections; chem formulas inside them
       are split into subscript elements, so match on the paragraph's full text content. */
    const lewisSection = waterAnalysis.explanation!.sections.lewis;
    expect(screen.getByText((_, element) => element?.tagName === "P" && element.textContent === lewisSection)).toBeInTheDocument(); });

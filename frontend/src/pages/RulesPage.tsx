...
export default function RulesPage() {
  ...
  return <PageContainer><header className="page-intro"><p className="eyebrow">{t("rules.eyebrow")}</p><h1>{t("rules.title")}</h1><p>{t("rules.intro")}</p></header>{error && <p className="error-message">{error}</p>}<div className="rules-grid">{rules.map((rule) => <article className="rule-card" key={rule.ax_en}><GeometryIcon geometry={rule.molecular_geometry} label={geometryLabel(t, rule.molecular_geometry)} /><div><strong><ChemFormula text={rule.ax_en} /></strong><p>{rule.bonding_domains} {t("rules.bondingDomains")}<br />{rule.lone_pair_domains} {t("rules.lonePairs")}</p><dl><dt>{t("rules.electronDomain")}</dt><dd>{geometryLabel(t, rule.electron_geometry)}</dd><dt>{t("rules.molecular")}</dt><dd>{geometryLabel(t, rule.molecular_geometry)}</dd><dt>{t("rules.angle")}</dt><dd>{rule.ideal_angle}</dd><dt>{t("rules.example")}</dt><dd>{examples.filter((item) => item.ax_en === rule.ax_en).length ? examples.filter((item) => item.ax_en === rule.ax_en).map((item, index) => <span key={item.formula}>{index > 0 && ", "}<ChemFormula text={item.formula} /></span>) : "—"}</dd></dl><small>{t(`rules.teachingNote.${rule.ax_en}`)}</small></div></article>)}</div></PageContainer>;
}
